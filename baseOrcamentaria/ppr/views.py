...
@csrf_exempt
@api_view(['POST'])
def calculos_realizado_ppr(request):
...
@csrf_exempt
@api_view(['POST'])
def calculos_realizado_matriz(request):
    cc_list = request.data.get('ccs', [])
    filiais_list = request.data.get('filial', [])
    ano = request.data.get('ano', None)
    conta1 = '3401%'
...
###-------------------------Obtem e agrua por grupo de contas----------------------------## 
    ...
    total_variavel = (
        ...
        total_tipo_deb.get('Custo Direto Variável Matéria Prima', 0) +
        total_tipo_deb.get('Custo Direto Variável Embalagens', 0)
    )
